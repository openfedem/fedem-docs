...
The model reduction process for each FE part generates superelement mass- and
...
a stress recovery can be conducted on the mechanism.
...
for more information about the stress recovery calculations.
See \refSection{stress-recovery-analysis}{Stress recovery analysis}
to setup and perform the stress recovery.
...
A mode shape recovery enables you to expand the system mode shapes
...
to setup and perform the mode shape recovery.
...
for more information about the strain rosette recovery calculations.
See \refSection{strain-rosette-analysis-1}{Strain rosette analysis}
to setup and perform the strain rosette recovery.
...
in a similar manner as in the strain rosette recovery.
...
to setup and perform the strain coat recovery.
...
This is handy if you only want to asses the static properties of an FE part
...
\Caution{Many of the solver options listed in
  ...
  Consult the OpenFEDEM team if you are in doubt
...
Fedem has the capability of running its solvers also on individual parts of the
mechanism (FE parts and element groups).
This is beneficial when dealing with big models containing a large number of
FE parts, where the solver execution may be both time- and disc-space consuming.
...
\begin{minipage}{0.45\textwidth}
  \raggedright
  All the solver processes listed in
  \refSection{overview-of-fedem-analyses}{Overview of Fedem analyses}
  may be run on FE parts, except for the Dynamics analysis which only operates
  on the global mechanism assembly.
  A part-wise solve process is started by right-clicking on an FE part in the
  ...
  more FE parts simultaneously. As always, when trying to run the Reducer,
  an FE part will only be reduced if necessary.
\end{minipage}%
...
To assess the dynamic properties of the resulting superelement matrices,
...
\item You can enable/disable both the
  ...
  geometric stiffness on FE parts, see the
  ...
  the inertia forces on FE parts with only a few Triads that experience
  high-speed rotations (see the
...
While using default settings, all primary variables but no secondary variables
...
switch on the output of all secondary variables available in the model.
...
Similarly, the strain rosette recovery can also be performed
...
\IconTextFirst{stress}{
  ...
  of FE parts or on individual element groups, see
...
\begin{minipage}{0.53\textwidth}
  ...
  \item{\sl Mode shapes to recover:} --
    List of selected mode shapes to expand during post-processing
...
\IconTextFirst{modes}{
  ...
  tool bar (or from the \textbf{Solve} menu).
  You may also run the mode shape recovery on a selection of parts, see
  \refSection{part-and-group-wise-solving}{Part- and group-wise solving}.}

When Fedem has post-processed all the selected modes, you can animate the
...
\IconTextFirst{gage}{
  ...
  You may also run the strain rosette recovery on a selection of parts,
  see \refSection{part-and-group-wise-solving}{Part- and group-wise solving}.
  This command will ignore parts which do not have any Strain Rosette objects
  defined on them.}
...
object {\sl ID}. This file is output in the nCode DAC format.
...
\begin{wrapfigure}[20]{r}{0.35\textwidth} \vskip-6mm
  \includegraphics[width=0.35\textwidth]{Figures/6-GenerateStraincoat}
\end{wrapfigure}

Before you run the strain coat analysis, you \newline must generate
strain coat elements for the parts \newline or element groups in question.
This is done by \newline right-clicking the parts or groups in the
{\sl Objects} \newline list of the Model Manager panel and selecting \newline
\textbf{Generate Strain Coat} from the menu. \newline
...
\IconTextFirst{fppSetup}{
  ...
  tool bar (or from the \textbf{Solve} menu).
  The Strain Coat Recovery is then performed on each FE part in the model
...
\begin{wrapfigure}[18]{r}{0.5\textwidth}
  \includegraphics[width=0.5\textwidth]{Figures/6-SolveLinearAnalysis}
\end{wrapfigure}

Start the process by right-clicking on the FE part (or parts) in the \newline
{\sl Objects} list of the Model Manager \newline panel,
and choose \textbf{Solve} and then \newline \textbf{Linear Analysis}
from the menu \newline (as shown to the right).

The results of a linear analysis is only the displacement field,
which then can be viewed in an Animation object.
However, it is possible to perform a part-wise stress recovery on the same part
afterwards, to compute the stresses based on the computed displacement field.
This stress recovery will perform the same simulation as the normal
stress recovery process described in
...
Recovery analyses exported to a single RPC-file or a multi-columns ASCII file),
...
It is possible to post-process the simulation results (graph plotting and
