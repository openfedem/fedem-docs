...
See also \refSection{property-editor}{Property Editor}.
...
in future releases. However, many of the features listed here will be supported
...
Simulating large models over long time series will generate a lot of results.
Users may then be interested in the results data for only a few objects of a
certain type, in order to reduce the size of the result database files.

To facilitate such output selection, {\tt-allPrimaryVars-}
may be specified as additional option for the Dynamics Solver (see
\refSection{additional-solver-options}{Additional solver options}),
to switch off all output of primary solution variables.
The following description field command is then provided for specific objects:
...
It has no effect for the other object types.

The easiest way to request output of all secondary variables in the model is
to specify {\tt-allSecondaryVars} as additional option for the Dynamics Solver,
if the size of the results database is of no concern.
However, requesting output of only a subset of the available variables on only
a few objects may be done through the following description field command:
...
The number of flags in the {\tt\#saveVar} command varies depending on
the type of object it is used for. The interpretation of the flags
for different object types is given in the following.
...
They are used for features that are still under development or testing,
or as a back-door for some special-purpose features.
For the sake of completeness, these commands are listed in the table below.
...
\begin{tabular}{|>\raggedright p{0.22\textwidth} | p{0.27\textwidth} | p{0.4\textwidth}|}
  ...
  \hline\hline
  {\tt\#UseEngines} & Mechanism & Applied in Model Preferences \\
  \hline
  ...
  {\tt\#Slam}            & Generic Part & \\
  \hline
  {\tt\#LocalAxis} & Triad & For pre-R7.3 compatibility \\
  \hline
  {\tt\#ShowDir}   & Triad & \\
  \hline
  {\tt\#SysDir}    & Triad & \\
  ...
  \hline
  {\tt\#Acceleration \#Velocity} & Spring & Prescribed motion \\
  \hline
  ...
  \hline
  {\tt\#OuterPipeRadius \#PipeRadius}   & Friction & DrillSim support \\
  \hline
  {\tt\#HydroFric \#RadFric \#SkinFric} & Friction & DrillSim support \\
  \hline
  {\tt\#BALL\_FRICTION \#BALL\_FRICTION2} & Rotational Friction & \\
  \hline
\end{tabular}
\begin{tabular}{|>\raggedright p{0.23\textwidth} | p{0.26\textwidth} | p{0.4\textwidth}|}
  \hline
  \rowcolor[HTML]{EFEFEF}
  \rule{0pt}{15pt} Command & Object Type & Comment \\
  \hline\hline
  {\tt\#FlipWCaxis \#ISWTCH \#MirrorTChar \#SteadyState} & Tire & \\
  \hline
  {\tt\#radialDamp \#radialStiff} & Tire & \\
  \hline
  {\tt\#RoadXengId \#RoadYengId \#RoadZengId} & Road & \\
  \hline
  ...
  \hline
  {\tt\#JONSWAP \#PiersonMoskowitz} & General function & \\
  \hline
  {\tt\#EndTime} & General function &  \\
  \hline
  {\tt\#ModalDamping} & General function & \\
  \hline
  {\tt\#ramp}   & General function & Only for function type Constant \\
  \hline
  {\tt\#noramp} & General function & Switch off initial ramping \\
  \hline
  {\tt\#Wear}   & Graph & \\
  \hline
  {\tt\#Model}  & Graph & \\
