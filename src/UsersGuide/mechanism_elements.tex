...
% Temporary, to aid setting picture environment sizes
...
\Note{The {\rm Constraint Type} of the {\rm Rz} DOF in the two Revolute joints
  of the gear has to be set to either {\rm Free} or {\rm Spring-Damper},
  see~\refSection{joint-variables}{Joint variables}.
  It can neither be {\rm Prescribed} nor {\rm Fixed}.}

\begin{wrapfigure}{r}{0.4\textwidth}
  \vspace{-7mm}
  \includegraphics[trim=0 10 0 0,clip,width=0.4\textwidth]{Figures/4-GearSymbol}
\end{wrapfigure}

\subsubsection{Gear symbol}

The gear symbol (shown at right) \newline
is displayed in the {\sl Modeler} view \newline
as a line between two revolute joints.

\begin{wrapfigure}[3]{r}{0.45\textwidth}
  \vskip2mm
  \includegraphics[trim=0 100 300 0,clip,width=0.45\textwidth]{\ReferenceImg/prp/prp-gear-1}
\end{wrapfigure}

\SubSubSection{Transmission ratio}{transmission-ratio}

You can specify the gear {\sl Transmission output ratio} (dimensionless rate)
in the Property Editor panel (shown at right). For information about the
transmission ratio in gears, see the \FedemTGuide{Section 6.4.1, "Gear joint"}.
...
\hspace*{0pt}\Note{
  The {\rm Constraint Type} of both the {\rm Rz} DOF in input Revolute joint
  and the {\rm Tz} DOF in the output Prismatic joint have to be set to either
  {\rm Free} or {\rm Spring-Damper},
  see~\refSection{joint-variables}{Joint variables}.
  They can neither be {\rm Prescribed} nor {\rm Fixed}.}

\begin{wrapfigure}{r}{0.38\textwidth}
  \vspace{-9mm}
  \includegraphics[width=0.38\textwidth]{Figures/4-rack-and-pinion-symbol}
\end{wrapfigure}

\subsubsection{Rack-and-Pinion symbol}

The Rack-and-Pinion symbol (shown at right) is displayed in the {\sl Modeler}
view as a line between a prismatic joint and a revolute joint.

\clearpage
\subsubsection{Transmission ratio}

You can specify the {\sl Transmission output ratio} (dimensionless rate) for a
Rack-and-Pinion constraint in the Property Editor panel
(see \refSubSection{transmission-ratio}{Transmission ratio}{gear}).
...
\begin{wrapfigure}[6]{r}{0.33\textwidth}
  \vspace{-6mm}
  \includegraphics[trim=0 45 0 0,clip,width=0.33\textwidth]{Figures/4-gen-joint-pair-constr}
\end{wrapfigure}

A joint pair constraint is created by \newline using the {\sl Type} keyword
...
\noindent\includegraphics[width=\textwidth]{\ReferenceImg/prp/math-expression-1}
...
\noindent
\begin{minipage}{0.45\textwidth}
  \includegraphics[width=\textwidth]{Figures/4-Functions_SmoothTrajectory}
\end{minipage}%
\begin{minipage}{0.55\textwidth}
...
\begin{wrapfigure}[5]{r}{0.35\textwidth}
  \vspace{-5mm}
  \includegraphics[width=0.35\textwidth]{Figures/4-TimeHistoryInputFile}
\end{wrapfigure}

To create a {\sl Time history input file} object, right click in the
Model Manager {\sl Objects} list, select \textbf{Create} and
\textbf{Time history input file}, as shown to the right.
A new object will then be created and its properties are displayed
...
There are two main types of sensors, simple sensors and relative sensors:
...
\IconTextFigure{simpleSensor}{
  ...
  particular object is being measured.
}{Figures/4-SimpleSensor}
\newline The symbol for a simple sensor is displayed in the {\sl Modeler} view,
\newline as shown to the right.
  
\SubSection{Relative sensors}{relative-sensors}

\vskip\parskip
\IconTextFigure{relativeSensor}{
  {\sl Relative sensors} are used to tag two (or four) triads in order to
  extract relative measurements from them. The sensor will be displayed in the
  {\sl Argument} drop-down menu in Functions and the {\sl Input} menu in Control
  Inputs. The symbol for a relative sensor is displayed in the
  {\sl Modeler} view, as shown to the right.
}{Figures/4-RelativeSensor}

If four triads are tagged in a relative sensor, the sensor will measure the
angle between the two vectors defined by the first and third triad, and the
second and fourth triad, respectively.
The first and second triad may in this case be colocated or be the same triad.
Similarly, the third and fourth triad may be the same or colocated, if the first
and second triads at the same time are at different locations.
...
\IconTextFirst{strainRosette}{
  Strain rosette objects are used to recover stresses and strains on particular
  spots in your mechanism. They can be placed on any of the FE models in the
  mechanism, both before and after solving the dynamics.
  The strain rosette recovery (see \refSection{strain-rosette-analysis-1}
  {Strain rosette analysis}) calculates strain rosette data for one FE model
  at a time. The output from such recovery is similar to the output from real
  strain gages, in addition to standard strain and stress quantities like
  Von Mises, principal stresses/strains, etc.
  See also the \FedemTGuide{Section 9.5, "Virtual strain gauges"}
  for the theoretical basis of strain rosettes.
  It is also possible to perform the strain rosette recovery during the dynamics
  simulation itself, see \refSubSection{advanced-tab}{Advanced tab}
  {part-properties}, if the strain rosettes are defined before starting the
  simulation.}

Strain rosettes are defined by selecting 3 or 4 FE nodes and the direction of
the first leg of the rosette. The nodes are selected by either clicking on the
FE model in the modeller view, or by directly typing their node numbers in the
interactive odometer that will pop up,
see \refSection{interactive-odometer}{Interactive Odometer and 3D Point Marker}.
The nodes defines a strain element in which the strains and stresses is
calculated at the centroid of the element based on the deformation of the nodes.
Material properties and shell thickness are extracted from the underlying FE
...
As the picture to the right indicates, \newline
the strain rosette symbol consists of different \newline parts
that visualize several aspects of the \newline virtual strain rosette.
...
\item{\sl Nodes} --
  This field displays the 3 or 4 FE node numbers used by the strain rosette.
  The \textbf{Edit} button lets you select a different set of nodes,
  in a similar manner as when creating the strain rosette object.
...
\noindent
\begin{picture}(343,58)
  \put(0,0){\includegraphics[trim=0 13 0 0,clip,width=\textwidth]{Figures/4-Generic_DB}}
  \put(63,36){\Bullet{1}}
  \put(75,15){\Bullet{2}}
\end{picture}
